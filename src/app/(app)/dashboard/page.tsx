      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Last 10 entries</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/log#all" className="text-xs text-slate-500 hover:text-brand-600 font-medium">
          ...
          <>
            ...
            {logs.length > 10 && (
              <div className="px-5 py-3 border-t border-slate-100 bg-slate-50">
                <Link href="/log#all" className="text-xs text-brand-600 hover:text-brand-700 font-medium flex items-center justify-center gap-1">
                  View all {logs.length} logs on Log page →
                </Link>
              </div>
            )}
          </>
  ...
  return (
    <div className="group px-5 py-4 hover:bg-slate-50/70 transition-colors flex gap-4 items-start">
      <div className="flex-shrink-0 text-center bg-slate-100 rounded-lg px-2.5 py-2 min-w-[52px]">
        <div className="text-[10px] text-slate-500 font-medium uppercase tracking-wide">{format(parseISO(log.date), 'MMM')}</div>
        <div className="text-lg font-bold text-slate-800 leading-none">{format(parseISO(log.date), 'd')}</div>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap gap-1.5 mb-1.5">
          ...
          {log.manager && <span className="badge bg-amber-100 text-amber-700">👤 {log.manager}</span>}
        </div>
        <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{log.description}</p>
      </div>
      <div className="flex-shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <Link href={`/log?edit=${log.id}`} className="btn-ghost hover:text-brand-600"><Edit2 size={15} /></Link>
        <button onClick={() => onDelete(log.id)} className="btn-ghost hover:text-red-500"><Trash2 size={15} /></button>
      </div>
    </div>
  )
}

function StatCard({ icon: Icon, label, value, color }: { icon: any; label: string; value: number; color: 'blue'|'violet'|'emerald' }) {
  const colors = { blue: 'bg-blue-50 text-blue-600 border-l-blue-500', violet: 'bg-violet-50 text-violet-600 border-l-violet-500', emerald: 'bg-emerald-50 text-emerald-600 border-l-emerald-500' }
  return (
    <div className={`card flex items-center gap-4 p-5 border-l-4 ${colors[color]}`}>
      <div className={`p-3 rounded-xl ${colors[color].split(' ').slice(0,2).join(' ')}`}><Icon size={22} /></div>
      <div>
